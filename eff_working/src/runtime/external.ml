...
(* let from_elt e = V.Const (Const.of_elt e) *)
...
(* let value_elt c  = V.Value (from_elt c) *)

let value_char c  = V.Value (from_char c)
...
let string_operations =
  ...
  ; ( "get", binary_closure (fun v1 v2 -> value_char (String.get (V.to_str v1) (V.to_int v2) )))
  ; ( "char_to_string", from_fun (fun v -> value_str (String.make 1 (V.to_char v))) ) 
  
  
  (* ; ( "escaped", from_fun (fun v -> value_str (Char.escaped (V.to_char v))) )   *)
  ]
